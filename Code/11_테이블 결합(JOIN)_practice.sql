...
-- LEFT JOIN: 두 테이블의 공통 값이 매칭되는 데이터만 결합
-- (계속) 왼쪽 테이블의 매칭되는 않는 데이터는 NULL
...
SELECT  *
  ...
  JOIN  SALES AS B
    ON  A.MEM_NO = B.MEM_NO;
-- (결과설명) NULL은 회원가입만하고 주문은 하지 않는 회원을 의미



-- RIGHT JOIN: 두 테이블의 공통 값이 매칭되는 데이터만 결합
-- (계속)오른쪽 테이블의 매칭되는 않는 데이터는 NULL
-- (계속) IS NULL: 비교 연산자 / NULL인 값만 반환
...
-- 임시테이블 조회
-- (설명) 임시테이블(TEMPORARY TABLE)은 서버 연결 종료시 자동으로 삭제됨
...
-- 3개 이상 테이블 결합
-- 주문(Sales) 테이블 기준으로, 
-- (계속) 회원(Customer) 및 상품(Product) 테이블 LEFT JOIN 결합
SELECT  *
  FROM  SALES AS A
  LEFT
  JOIN  CUSTOMER AS B
